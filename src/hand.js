...
export default class Hand {
  ...
  updateFinger(finger, data) {
    if (data) {
      ...
      var metacarpal = {
        'pos': BABYLON.Vector3.Center(carp, mcp)
      }; // bone 0
      var proximal = {
        'pos': BABYLON.Vector3.Center(mcp, pip)
      }; // bone 1
      var middle = {
        'pos': BABYLON.Vector3.Center(pip, dip)
      }; // bone 2
      var distal = {
        'pos': BABYLON.Vector3.Center(dip, tip)
      }; // bone 3
      ...
      this.bones[finger]['distal'].position = distal['pos'];

      this.bones[finger]['middle'].position = middle['pos'];

      this.bones[finger]['proximal'].position = proximal['pos'];
      if(finger!='thumb'){
        this.bones[finger]['direction'].position = direction;
      }
    }
  }
  /*
  rotateBones() {
      for each(var bone in this.bones) {
        var mat = bone.getLocalMatrix();
        mat = mat.multiply(BABYLON.Matrix.RotateX(this.rotation_x);
        mat = mat.multiply(BABYLON.Matrix.RotateZ(this.rotation_z);
